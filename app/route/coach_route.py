...
@coach_route_bp.route('/coaches/<int:coach_id>/teams', methods=['GET'])
def get_coach_teams(coach_id):
    sort_order = request.args.get('sort_order', 'asc')
    page = request.args.get('page', 1, type=int)
    per_page = request.args.get('per_page', 5, type=int)

    teams, total = coach_service.get_coach_teams(coach_id, sort_order, page, per_page)
    return create_response(teams, "Teams found", 200, True)

@coach_route_bp.route('/teams/<int:team_id>/coaches', methods=['GET'])
def get_team_coaches(team_id):
    sort_order = request.args.get('sort_order', 'asc')
    page = request.args.get('page', 1, type=int)
    per_page = request.args.get('per_page', None)

    coaches = coach_service.get_coaches_team(team_id, sort_order, page, per_page)
    return create_response(coaches, "Coaches found", 200, True)
